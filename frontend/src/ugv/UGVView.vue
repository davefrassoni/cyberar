...
const ZONE_NAMES = { atlantic: 'ISLA AURORA', 'vaca-muerta': 'CORREDOR DE DUCTOS', 'triple-frontera': 'PASO FRONTERIZO' };
const zoneName = computed(() => ZONE_NAMES[props.state.scenario_meta?.key] || 'ZONA DE PATRULLA');
const bounds = computed(() => {
  const xs = ugv.value.route.map(p => p.x), ys = ugv.value.route.map(p => p.y);
  return { minX: Math.min(...xs), maxX: Math.max(...xs), minY: Math.min(...ys), maxY: Math.max(...ys) };
});
const point = p => {
  const b = bounds.value;
  const spanX = Math.max(1, b.maxX - b.minX), spanY = Math.max(1, b.maxY - b.minY);
  return { x: 90 + ((p.x - b.minX) / spanX) * 620, y: 90 + ((p.y - b.minY) / spanY) * 300 };
};
...
<template>
  <section class="ugv-view">
    <div class="map-top"><div><span class="eyebrow">02 / PATRULLA TERRESTRE</span><h2>{{ zoneName }} <span>{{ ugv.asset }}</span></h2></div><span class="tiny">100% SIMULADO</span></div>
    <div class="ground-scene">
      <svg viewBox="0 0 800 480" role="img" :aria-label="`${ugv.asset} animado recorriendo su patrulla en ${zoneName}`">
        ...
        <g class="ground-vehicle" :style="{ transform: `translate(${position.x}px, ${position.y}px)`, '--travel-duration': `${1/state.speed}s` }">
          ...
          <text x="-23" y="-48" fill="#deeadf" font-size="12" font-weight="bold">{{ ugv.asset }}</text>
        </g>
